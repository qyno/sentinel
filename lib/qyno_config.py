...
class QynoConfig():
    ...
    def slurp_config_file(self, filename):
        # read qyno.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is qyno.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from qyno.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Qyno defaults...
        default_port = 37111 if (network == 'mainnet') else 37113

        # use default port for network if not specified in qyno.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from qyno.conf
