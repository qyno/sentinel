...
class QynoDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some qynod methods
    ...
    @classmethod
    def from_qyno_conf(self, qyno_dot_conf):
        from qyno_config import QynoConfig
        config_text = QynoConfig.slurp_config_file(qyno_dot_conf)
        creds = QynoConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = qynolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = qynolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = qynolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_qynod_proto_version_with_sentinel_ping)
