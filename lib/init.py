...
def has_qyno_conf():
    ...
    valid_qyno_conf = False

    # ensure qyno_conf exists & readable
    #
    # if not, print a message stating that Qyno Core must be installed and
    # configured, including JSONRPC access in qyno.conf
    try:
        f = io.open(config.qyno_conf)
        valid_qyno_conf = True
    except IOError as e:
        print(e)

    return valid_qyno_conf
...
def main():
    ...
    if not has_qyno_conf():
        print("Qyno must be installed and configured, including JSONRPC access in qyno.conf")
        sys.exit(1)
