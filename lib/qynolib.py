...
def is_valid_qyno_address(address, network='mainnet'):
    ...
    qyno_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Qyno address
        return False

    if (address_version != qyno_version):
        return False
...
# shims 'til we can fix the qynod side
def SHIM_serialise_for_qynod(sentinel_hex):
    from models import QNOD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = QNOD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in qynod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # qynod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    qynod_hex = serialise(obj)
    return qynod_hex


# shims 'til we can fix the qynod side
def SHIM_deserialise_from_qynod(qynod_hex):
    from models import QNOD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(qynod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('qyno.conf')
