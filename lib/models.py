...
class GovernanceObject(BaseModel):
    ...
    # sync qynod gobject list with our local relational DB backend
    @classmethod
    def sync(self, qynod):
        golist = qynod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_qynod(qynod, item)
    ...
    @classmethod
    def import_gobject_from_qynod(self, qynod, rec):
        import decimal
        import qynolib
        import inflection
        ...
        # shim/qynod conversion
        object_hex = qynolib.SHIM_deserialise_from_qynod(object_hex)
        objects = qynolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from qynod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from qynod, with every run
        ...
        # get/create, then sync payment amounts, etc. from qynod - Qynod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(qynod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from qynod! %s" % e)
            govobj.vote_delete(qynod)
            return (govobj, None)
    ...
    def vote_delete(self, qynod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(qynod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, qynod, signal, outcome):
        import qynolib
        ...
        output = qynod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = qynolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(qynod, signal)

    def sync_network_vote(self, qynod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = qynod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = QNOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 qyno addr, non-multisig
            if not qynolib.is_valid_qyno_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Qyno address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (qynolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/QynoDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import qynolib
        obj_data = qynolib.SHIM_serialise_for_qynod(self.serialise())
    ...
    def prepare(self, qynod):
        try:
            object_hash = qynod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = QNOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not qynolib.is_valid_qyno_address(addr, config.network):
    ...
    def hash(self):
        import qynolib
        return qynolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = QNOD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, qynod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - qynod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, qynod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - qynod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, qynod):
        now = int(time.time())
        return (self.created_at < (now - qynod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, qynod):
        if self.is_expired(qynod):
            return False

        return True

    def is_deletable(self, qynod):
        if self.is_expired(qynod):
            return True
